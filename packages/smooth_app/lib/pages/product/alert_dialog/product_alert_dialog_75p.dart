...
class AlertDialog75p extends StatefulWidget {
  const AlertDialog75p(this.product);
  final Product product;
  @override
  _AlertDialog75pState createState() => _AlertDialog75pState();

}

class _AlertDialog75pState extends State<AlertDialog75p> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: Column(children: <Widget>[
        const Text(
            'If you consume at this portion'),
        const SizedBox(
          height: 10,
        ),
        const Text(
            'Your amount of nutrient left will be'),
        energyCard(),
        sodiumCard(),
        proteinCard(),
        carbohydrateCard(),
  ...
  Widget energyCard() {
    ...
    var energyPerday = bmr,
        energyConsume = double.parse(_useNutritionData['calorie'].toString()) +
            (widget.product.nutriments.energyKcal * 3 / 4);
  ...
  Widget proteinCard() {
    ...
    var proteinPerDay = proteinList.reduce(min).round(),
        _proteinConsume = double.parse(_useNutritionData['protein'].toString()) + (widget.product.nutriments.proteinsServing * 3 / 4);
  ...
  Widget carbohydrateCard() {
    ...
    var carbohydratePerDay = carbohydrateList.reduce(min).round(),
        _carbohydrateConsume =
            double.parse(_useNutritionData['carbohydrate'].toString()) + (widget.product.nutriments.carbohydratesServing * 3 / 4);
  ...
  Widget fatCard() {
    ...
    final fatPerDay = fatList.reduce(min).round(),
        ...
        _fatConsume = double.parse(_useNutritionData['fat'].toString()) + (widget.product.nutriments.fatServing * 3 / 4),
        _satFatConsume = double.parse(_useNutritionData['fat'].toString()) + (widget.product.nutriments.saturatedFatServing * 3 / 4),
  ...
  Widget sodiumCard() {
    ...
    var sodiumPerDay = sodiumList.reduce(min).round(),
        _sodiumConsume = int.parse(_useNutritionData['fat'].toString()) + (widget.product.nutriments.sodiumServing * 1000 * 3 / 4).round();
    return SmoothCard(
